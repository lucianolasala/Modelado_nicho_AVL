...
class(elevation)
elevation  # loads 1  of  4  bands (flow_acc)
...
elev_cropped <- crop(elevation_stack, G)
plot(elev_cropped$X1)


# Mask raster stack using the vector

elev_mask <- mask(elev_cropped, G)
...
for(i in 1:length(variables)) {
  writeRaster(individual_elevation[[i]], filename = paste0("C:/Users/User/Documents/Analyses/AVL/Rasters/Projection_area/", variables[i]), format = "ascii")
}
